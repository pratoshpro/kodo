(function () {
  ...
  inputSearch.addEventListener("change", onChangeHandler);
  sortDropdown.addEventListener("change", onChangeDropdown);
  ...
  function filterList(filterText) {
    ...
    if (sortDropdown.value) {
      data = data.sort(function (a, b) {
        return SortByKey(sortDropdown.value,a[sortDropdown.value] ,b[sortDropdown.value]);
      });
    }

    renderItem(data);
  }
  function SortByKey(key,dataA,dataB){
    switch(key){
      case 'dateLastEdited':
  ...
  function renderItem(list) {
    ...
    list.forEach((item) => {
      ...
      descriptionElement.innerText = `title:${item.name}
      description:${item.description}`;
      element.appendChild(imgElement);
      element.appendChild(descriptionElement);
      element.classList.add("item-grid");
